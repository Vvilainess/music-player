...
const RecentPlayed = ({ title, data }) => {
    return (
        <>
            <Title className="capitalize font-bold text-2xl text-white">
                {title}
            </Title>
            <div className="grid gap-x-6 xs:grid-col-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 xxl:grid-cols-6 py-5">
                {data?.map((item) => {
                    return item.track.type === "artist" ? (
                        <SearchResultItem
                            key={item.track.id}
                            item={item.track}
                        />
